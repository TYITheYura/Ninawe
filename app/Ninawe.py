...
if "--worker" not in sys.argv:
    print("[Log] [CrashHandler] | Starting handler...")
    ...
    while True:
            ...
            try:
                ...
                if not rawData:
                    if time.time() - startTime > 60:
                        print(
                            "[Log] [CrashHandler] |" +
                            "Looks like the shell failed to start within 60s. Terminating..."
                        )
                        workerProcess.kill()
                        break
                else:
                    ...
                    if isHung:
                        print(
                            "[Log] [CrashHandler] | " +
                            "Looks like the shell is having a bad time and is completely frozen. Terminating worker..."
                        )
        ...
        if exitCode == 0:
            print("[Log] [CrashHandler] | Worker exited gracefully. Handler going dark.")
            shmem.close()
            sys.exit(0)
        ...
        if len(crashes) >= 3:
            ...
            print(f"[Log] [Kernel Panic] | Restarting frozen/dead shell (Crash {len(crashes)}/3)...")
    ...
    ''')
            ...
            try:
                ...
                print(f"[Ninawe] Failed to write HWND to shmem: {e}")
